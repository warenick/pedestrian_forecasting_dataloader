...
#
# dataset = DatasetFromTxt(path_, files, cfg, use_forces=True, forces_file=path_+"forces_19.02.txt")
...
model_name="trajPred1"
writer, experiment_name, best_model_path, folder_path = setup_experiment(model_name, "../logs/"+model_name, "trajPred1")
        ...
        if data.tgt_avail[i, -1]:
            ...
            if cov is not None:
                ...
                for j in range(1,4):
                    ...
                    r_cov_min = data.raster_from_agent[i] @ np.array([predictions[i, 0] - r_cov_x[0],
                                                                      predictions[i, 1] - r_cov_y[1],
                                                                     1])
                    r_cov_max = data.raster_from_agent[i] @ np.array([predictions[i, 0] + r_cov_x[0],
                                                                      predictions[i, 1] + r_cov_y[1],
                                                                      1])
            ...
            if traj is not None:
                ...
                for pose in traj[i]:
                    pose_raster = data.raster_from_agent[i] @ np.array([pose[0], pose[1], 1.])
                    r = 2
...
def validation(model, loader, epoch):
    ...
    for data in tqdm(loader):
        ...
        predictions, kl, traj = model(data.image, data.history_positions.float().cuda())
...
for epoch in range(epoch, 200):
    ...
    for i, data in enumerate(pbar):
        ...
        mask_goal = data.tgt_avail[:, -1] == 1
        mask_future = data.tgt_avail == 1
        predictions, kl, traj = model(data.image, data.history_positions.float().cuda(), goal=data.tgt[:, -1, :].float().cuda())
        ...
        dist_err = distance(predictions.mean, data.tgt[:, -1, :].cuda(), mask_goal.cuda())  # , data.tgt_avail[:,-1]
        kl_weight = min(((epoch ** 2 + 1) * 0.01), 10)
        loss = 5*ade + 0.1 * cov_size_loss + kl_weight * kl + 1 * nll_ + 10 * (dist_err)  # , data.tgt_avail[:,-1]
